...
#2
Выявляена еще одна проблема при работе с "Instance Group":
...
- в блоке "yandex_compute_instance_group.ig-1.instance_template.network_interface" предусмотрено поле "ip_address"
  ...
  хотя таже самая идея прекрасно работает для назначения hostname создаваемых ВМ с помощью конструкции вида 'hostname = "host{instance.index}"'
...
- возможно я не прав и скорее всего можно было бы обойти эту проблему используя DNS имена (FQDN) которые назначаются хостам внутри облака,
  но это решение НЕ универсальное и привязано к конкретному облачному провайдеру, а цель была в разработке универсальной конфигурации
  чтобы созданные хосты, могли быть в любом облаке и без проблем подключаться к Zabbix Серверу который вообще может быть физическим, а не виртуальным
  и находится где угодно, а не внутри облака "Yandex Cloud" (или облака любого другого провайдера)
