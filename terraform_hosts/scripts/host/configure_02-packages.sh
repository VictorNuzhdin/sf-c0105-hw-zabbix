...
sudo apt update -y
#sudo apt install -y whois                      ## E: Unable to locate package whois ..странно, недавно устанавливал и все норм было
sudo apt install -y whois
echo "" >> $LOG_PATH
whois --version | head -n 1 >> $LOG_PATH
which whois >> $LOG_PATH
whereis whois >> $LOG_PATH
echo "" >> $LOG_PATH
mkpasswd --version | head -n 1 >> $LOG_PATH
which mkpasswd >> $LOG_PATH
...
echo '## Installing net-tools package (includes "netstat") ..' >> $LOG_PATH
sudo apt install -y net-tools
echo "" >> $LOG_PATH
netstat --version | head -n1 >> $LOG_PATH
which netstat >> $LOG_PATH
whereis netstat >> $LOG_PATH
echo "" >> $LOG_PATH
